...
// MARK: - AVPlayerItemInitService
open class AVPlayerItemInitServiceMock: AVPlayerItemInitService, Mock {
    ...
    open func getItem(media: PlayerMedia, loadedAssetKeys: [String]) -> AVPlayerItem {
        addInvocation(.m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(Parameter<PlayerMedia>.value(`media`), Parameter<[String]>.value(`loadedAssetKeys`)))
		let perform = methodPerformValue(.m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(Parameter<PlayerMedia>.value(`media`), Parameter<[String]>.value(`loadedAssetKeys`))) as? (PlayerMedia, [String]) -> Void
		perform?(`media`, `loadedAssetKeys`)
		var __value: AVPlayerItem
		do {
		    __value = try methodReturnValue(.m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(Parameter<PlayerMedia>.value(`media`), Parameter<[String]>.value(`loadedAssetKeys`))).casted()
		} catch {
			onFatalFailure("Stub return value not specified for getItem(media: PlayerMedia, loadedAssetKeys: [String]). Use given")
			Failure("Stub return value not specified for getItem(media: PlayerMedia, loadedAssetKeys: [String]). Use given")
		}
		return __value
    }
    ...
    fileprivate enum MethodType {
        case m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(Parameter<PlayerMedia>, Parameter<[String]>)

        static func compareParameters(lhs: MethodType, rhs: MethodType, matcher: Matcher) -> Bool {
            switch (lhs, rhs) {
            case (.m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(let lhsMedia, let lhsLoadedassetkeys), .m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(let rhsMedia, let rhsLoadedassetkeys)):
                guard Parameter.compare(lhs: lhsMedia, rhs: rhsMedia, with: matcher) else { return false } 
                guard Parameter.compare(lhs: lhsLoadedassetkeys, rhs: rhsLoadedassetkeys, with: matcher) else { return false } 
                return true 
            }
        }
        ...
        func intValue() -> Int {
            switch self {
            case let .m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(p0, p1): return p0.intValue + p1.intValue
            }
        }
    }
    ...
    open class Given: StubbedMethod {
        ...
        public static func getItem(media: Parameter<PlayerMedia>, loadedAssetKeys: Parameter<[String]>, willReturn: AVPlayerItem...) -> MethodStub {
            return Given(method: .m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(`media`, `loadedAssetKeys`), products: willReturn.map({ StubProduct.return($0 as Any) }))
        }
        public static func getItem(media: Parameter<PlayerMedia>, loadedAssetKeys: Parameter<[String]>, willProduce: (Stubber<AVPlayerItem>) -> Void) -> MethodStub {
            let willReturn: [AVPlayerItem] = []
			let given: Given = { return Given(method: .m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(`media`, `loadedAssetKeys`), products: willReturn.map({ StubProduct.return($0 as Any) })) }()
			let stubber = given.stub(for: (AVPlayerItem).self)
			willProduce(stubber)
			return given
        }
    }
    ...
    public struct Verify {
        ...
        public static func getItem(media: Parameter<PlayerMedia>, loadedAssetKeys: Parameter<[String]>) -> Verify { return Verify(method: .m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(`media`, `loadedAssetKeys`))}
    }
    ...
    public struct Perform {
        ...
        public static func getItem(media: Parameter<PlayerMedia>, loadedAssetKeys: Parameter<[String]>, perform: @escaping (PlayerMedia, [String]) -> Void) -> Perform {
            return Perform(method: .m_getItem__media_medialoadedAssetKeys_loadedAssetKeys(`media`, `loadedAssetKeys`), performs: perform)
        }
    }
